...
func (s *Service) ModifyHardware(vmId int,
	...
	if len(cpuPinning) > 0 {
		if err := s.DB.Model(&vm).Update("cpu_pinning", cpuPinning).Error; err != nil {
			return fmt.Errorf("failed_to_update_vm_cpu_pinning_in_db: %w", err)
		}

		bhyveArgs := parsed.BhyveCommandline.Args
		for i, arg := range bhyveArgs {
			if len(arg.Value) > 2 && arg.Value[:2] == "-p" {
				bhyveArgs = append(bhyveArgs[:i], bhyveArgs[i+1:]...)
				break
			}
		}
